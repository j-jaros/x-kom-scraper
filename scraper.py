...
def main(primary_url):
    ...
    cpu_cooler = components.CpuCooler()
    gpu = components.Gpu()
    fullFilled = components.FullFilled()
    unrecognized = []
        ...
        for component in CLASSES:
            if component.title is not None:
                ...
                for params in params_list:
                    title = re.findall(rf'<div class="{COMPONENTS_PARAMETERS_KEYS["title_element"]}">(.*?)</div',
                                       params)
                    if not title:
                        title = re.findall(
                            rf'<div class="{COMPONENTS_PARAMETERS_KEYS["second_title_element"]}">(.*?)</div',
                            params)
                    title = title[0]
    ...
    def check_compatibility():
        ...
        if all([psu.title, gpu.title, cpu.title]) and int(psu.power) < int(
                gpu.power_consumption + cpu.power_consumption):
